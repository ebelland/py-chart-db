...
class TablePreviewPanel(QWidget):
    ...
    def _build_context_menu(self, pos: QPoint) -> "QMenu | None":
        ...
        trailing_items: tuple[MenuItem | None, ...] = (
            ...
            None,
            MenuItem(_("Ensure Hide column"), callback=self._ensure_hide,icon="hide"),
            MenuItem(_("Reset Hide to 0"), callback=self._reset_hide, icon=""),
            MenuItem(_("Invert Hide 0 <-> 1"), callback=self._invert_hide,icon="invert"),
            None,
            MenuItem(_("Ensure ClusterId column"), callback=self._ensure_cluster,icon="clustering"),
